...
public class LoginPanel extends JPanel {
    ...
    public LoginPanel(MainPanel mainPanel, ClientController controller) {
    ...
    public final void initComponents(ClientController controller) {
        ...
        play.addActionListener(e -> {
            if (usernameTextField.getText().isEmpty()) {
                ...
                if (controller.login(usernameTextField.getText())) {
                    mainPanel.updatePanel(Constants.GAME_KEY);
                } else {
                    new MessageDialog("SERVIDOR DESCONECTADO").showPopUp();
                }

            }
        });
        backToMenu.addActionListener(e -> mainPanel.updatePanel(Constants.MENU_KEY));
    }
